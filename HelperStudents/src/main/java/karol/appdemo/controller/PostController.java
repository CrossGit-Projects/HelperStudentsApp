...
import karol.appdemo.repository.PostRepository;
import karol.appdemo.service.DeanGroupService;
import karol.appdemo.service.PostService;
...
public class PostController {
    ...
    public String postFormProfesor(Model model) {
        ...
        List<DeanGroup> tmpListDeanPost = deanGroupService.findAll();
        model.addAttribute("tmpListDeanPost", tmpListDeanPost);
    ...
    public String postFormaAll(Model model) {
        ...
        for (int i = 0; i < postList.size(); i++) {
            if (postList.get(i).getGroupDeanPost().equals(user.getGroupLab())) {
                tmpListPost.add(postList.get(i));
            }
        }
        model.addAttribute("postList", tmpListPost);
